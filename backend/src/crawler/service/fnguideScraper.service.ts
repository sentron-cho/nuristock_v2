...
export const getFnGuideConsensus = async (code?: string) => {
  ...
  try {
    ...
    const url = `${REST_API.FNGUIDE_MAIN}?pGB=1&gicode=A${code?.replace(
      "A",
      ""
    ...
    for (let year = start; year < end; year++) {
      const data = parseFnGuideByYear(tableHtml, year); // 예상치 가져오기
      if (data) {
